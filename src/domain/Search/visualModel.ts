...
import type { BaseNodeProperty, LocalizationKeyDTO, LocationDTO, Preset, PresetStore } from '@/model/types'
 ...
 * @param node 사라질 노드
 */
export const baseNodeEmit = (_node: TextNode) => {}
...
const lzTextOverlay = (
	...
	const _isSelected2 = figma.currentPage.selection.includes(node)
	if (test && isSelected) {
		data = {
			...test,
			id,
...
export const textOriginRegister = async (data: Awaited<ReturnType<typeof textKeyRegister>>) => {
	...
	for (const [key, visibleNodes] of Object.entries(data)) {
		...
		// base 노드 체크
		let _baseCheck = false
		let location: LocationDTO | undefined
		let xNode: SceneNode | undefined
		// 아이디 생성이 필요한지 확인
		//
...
export const onAutoSelectModeRequest = () => {
	return on(AUTO_SELECT_MODE_PAIR.RESPONSE_KEY, async (bool?: boolean) => {
		...
		const json = figma.root.getPluginData(SELECT_MODE.AUTO_SELECT_MODE) === 'true'
...
export const overlayRender = async () => {
	...
	for (const item of hasKey) {
		...
		if (selectedIds.length === 0) {
			const _node = lzTextOverlay(item, optionColorMap, backgroundFrame, { x, y }, keepTarget)
...
const setOverlayAction = async (acceptAction: keyof typeof SAVE_ACTION, option: PresetMetaData) => {
	...
	if (acceptAction === SAVE_ACTION.SUBTRACT) {
		...
		const metadata = selectedNodes.map(node => getFrameNodeMetaData(node as FrameNode)).filter(item => item != null)
 ...
 * 서버
 */
export const presetSave = (name: string, _baseNodeId?: string, serverSectionId?: string) => {
...
export const onSelectModeMain = () => {
	...
	on(RENDER_TRIGGER.SAVE_ACCEPT, async (_acceptAction: keyof typeof SAVE_ACTION, option: PresetMetaData) => {
		figma.currentPage.selection = []
