...
>(
  ...
  const chain = useMemo(() => {
    ...
    if (config.fn2) activeChain.push('fn2')
    if (config.fn3) activeChain.push('fn3')
    if (config.fn4) activeChain.push('fn4')
    if (config.fn5) activeChain.push('fn5')
    if (config.fn6) activeChain.push('fn6')
    if (config.fn7) activeChain.push('fn7')
    if (config.fn8) activeChain.push('fn8')
    if (config.fn9) activeChain.push('fn9')
    if (config.fn10) activeChain.push('fn10')
    return activeChain
  }, [config])
...
const _Example = () => {
  ...
  const { state, results } = useFp(1, {
    ...
    fn3: async (str: string) => parseInt(str, 10) * 2,
